...
const Student = class Student {
	constructor(first, last, gpa, degree) {
...
const Product = class Product {
     ...
     */
    static inStock = (products) => {return products.filter(x => x.availability == "In Stock")}
    ...
    static halfOff = (products) => {
		return products.map(x => new Product(`${x.name},${x.price * 0.5},${x.availability}`));
	}
    ...
    static printProducts = (products) => {
		products.forEach(x => {
			let avail = "No";
			if (x.availability == "In Stock") { avail = "Yes"; }
		
			console.log(`Product: ${x.name}, Cost: ${(new Intl.NumberFormat('en-Latn-US', { style: 'currency', currency: 'USD' }).format(x.price))}, Availability: ${avail}`);
		});
